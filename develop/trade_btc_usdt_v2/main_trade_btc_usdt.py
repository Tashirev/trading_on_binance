...
class ResNet1D(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(6, 64, kernel_size=70, stride=2, padding=3, bias=False)  # kernel_size = 4 по умолчанию 7
...
def sell_buy(volume_before, volume_x, x_y_price, x_usdt_price, y_usdt_price, part, FEE, SLIP_N, centralization):  # продаваемая валюта X, покупаемая валюта Y, ....

    # два случая: volume_x > volume/2, то нужно продавать больше, иначе нужно продавать меньше, так среднее будет тяготеть к центу
    if volume_x * x_usdt_price > volume_before/2:
        volume_x_sell = (volume_before/x_usdt_price) * part * (1+centralization)  # продаваемый объём в крипте
    else:
        volume_x_sell = (volume_before/x_usdt_price) * part * (1-centralization)  # продаваемый объём в крипте
        if volume_x_sell > volume_x:
            volume_x_sell = 0
...
def profit_calculation(coins, ticker_price, predict, part, fee, centralization, slip_n):
    ...
    volume_btc_sell = 0
    volume_usdt_sell = 0
    volume_before = volume_btc * ticker_price[0] + volume_usdt  # текущий объём кошелька в usdt
    fee_usdt = 0
    ...
    if predict == 1:   # продажа usdt, покупка btc
        volume_usdt_sell, volume_btc_buy, fee_usdt = sell_buy(volume_before, volume_usdt, 1 / ticker_price[0], ticker_price[0]/ticker_price[0], ticker_price[0], part, fee, slip_n, centralization)
        ...
        volume_btc_sell, volume_usdt_buy, fee_usdt = sell_buy(volume_before, volume_btc, ticker_price[0], ticker_price[0], ticker_price[0]/ticker_price[0], part, fee, slip_n, centralization)
...
def trading_status(price_1, price_2, start, part, fee, profit_fee_coef, trade_move_coef, centralization):
    trading_on = int((price_1-price_2)/price_1 < fee*trade_move_coef)
    if trading_on == 0: # сохранение блокировки в БД
        sql_trading_settings_write(datetime.fromtimestamp(time()), trading_on, start, part, fee, profit_fee_coef, trade_move_coef, centralization)

    return trading_on

def main():
    print('Binance trade btc_usdt v.2')
    ...
    while True:
        ...
        trading_on, start, part, fee, profit_fee_coef, trade_move_coef, centralization = read_settings_sql()
        ...
        if(len(tickers_prices) >= features_length):
            ...
            if start == 1:
                ...
                sql_trading_settings_write(datetime.fromtimestamp(time()), trading_on, start, part, fee,
                                           profit_fee_coef, trade_move_coef, centralization)
            ...
            if predict == 1:  # btc_usdt_min_trade_count *  part
                volume_before, fee_usdt, min_order_status, volume_status, volume_usdt_buy, volume_btc_buy, volume_usdt_sell, volume_btc_sell = profit_calculation(
                    coins, ticker_price, predict, part, fee, centralization, slip_n)
            ...
            if min_order_status + volume_status == 2: # сделка подходит по всем параметрам, все статусы 1
                ...
                if predict == 1:  # продажа usdt, покупка btc
                    ...
                    if trading_on == 1:  # проверка и изменение статуса только если он 1, 0 поменять нельзя (только вручную через БД)
                        trading_on = trading_status(ticker_price[0], trade_max_first, start, part, fee, profit_fee_coef, trade_move_coef, centralization)
                    ...
                    if trading_on == 1:  # проверка и изменение статуса только если он 1, 0 поменять нельзя (только вручную через БД)
                        trading_on = trading_status(trade_min_first, ticker_price[0], start, part, fee, profit_fee_coef, trade_move_coef, centralization)
                    if profit_fee_status and trading_on == 1:
            ...
            print('----trading----')
            print(f'time: {datetime.fromtimestamp(time_begin)}, predict: {predict}, trade: {volume_status, profit_fee_status, min_order_status}, order: {order["symbol"]}')
            print(f'trading_on: {trading_on}, part: {part}, fee: {fee}, profit_fee_coef: {profit_fee_coef}, trade_move_coef: {trade_move_coef}, centralization: {centralization}')
        else:
